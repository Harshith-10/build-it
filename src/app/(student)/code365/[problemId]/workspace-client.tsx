...
export default function WorkspaceClient({ problem, userId }: WorkspaceProps) {
  ...
  useEffect(() => {
    ...
    const savedCode = localStorage.getItem(getCodeStorageKey(problem.id, selectedLanguage));
    if (savedCode) {
      setCode(savedCode);
    } else {
      const defaultCode = (problem.driverCode as Record<string, string> | null)?.[selectedLanguage] || "";
      setCode(defaultCode);
    }
  ...
  const availableLanguages = Array.from(new Set(runtimes.map((r) => r.language))).sort();
        ...
        <ResizablePanelGroup orientation="horizontal" className="h-full">
              ...
              <div className="p-6">
                <div className="max-w-none grid grid-cols-[minmax(0,1fr)]">
                  <Markdown
                    ...
                    components={{
                      h1: ({ className, ...props }) => <h1 className={cn("scroll-m-20 text-3xl font-bold tracking-tight mb-4", className)} {...props} />,
                      h2: ({ className, ...props }) => <h2 className={cn("mt-8 scroll-m-20 border-b pb-1 text-2xl font-semibold tracking-tight first:mt-0", className)} {...props} />,
                      h3: ({ className, ...props }) => <h3 className={cn("mt-6 scroll-m-20 text-xl font-semibold tracking-tight", className)} {...props} />,
                      p: ({ className, ...props }) => <p className={cn("leading-7 [&:not(:first-child)]:mt-4", className)} {...props} />,
                      ...
                      pre: ({ className, ...props }) => (
                        <ScrollArea className="mb-4 mt-4 rounded-lg border max-w-full">
                          <pre className={cn("px-4 py-4 [&_code]:border-none [&_code]:bg-transparent [&_code]:p-0", className)} {...props} />
                          <ScrollBar orientation="horizontal" />
                        </ScrollArea>
                      ),
                      code: ({ className, ...props }) => (
                        <code className={cn("relative rounded border bg-primary/10 px-[0.3rem] py-[0.2rem] font-mono text-sm", className)} {...props} />
                      ),
                    }}
                  >
          ...
          <ResizablePanel defaultSize={60} minSize={30}>
            <ResizablePanelGroup orientation="vertical">
                ...
                <div className="flex h-full flex-col">
                  ...
                  <div className="relative flex-1 overflow-hidden text-[14px]">
                    <CodeMirror
                      ...
                      value={code}
                      height="100%"
                      extensions={[getLanguageExtension(selectedLanguage), EditorState.tabSize.of(4)]}
              ...
              <ResizableHandle className="w-full h-px" handleOrientation="horizontal" withHandle />
