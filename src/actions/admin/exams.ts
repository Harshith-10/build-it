...
import { and, asc, desc, eq, ilike, inArray, ne, or, sql, isNull } from "drizzle-orm";
...
export async function getSubmissionAuditLogs(assignmentId: string) {
  try {
    const assignment = await db.query.examAssignments.findFirst({
      where: eq(examAssignments.id, assignmentId),
      with: {
    ...
    const access = await ensureExamReadAccess(assignment.examId);
    if (!access.examRecord) {
      return { success: false, error: "Unauthorized" };
    }

    const events = await db.query.malpracticeEvents.findMany({
      where: eq(malpracticeEvents.assignmentId, assignmentId),
      orderBy: [asc(malpracticeEvents.createdAt)],
      with: {
        actor: {
          columns: {
            id: true,
    ...
    // Synthesize Exam Started
    if (assignment.startedAt || assignment.createdAt) {
      timeline.push({
        id: "sys-started",
        type: "exam_started",
        details: "Student initiated exam session",
        createdAt: assignment.startedAt || assignment.createdAt,
      });
    }

    // Add recorded malpractice & admin events
    ...
    // Synthesize Auto Terminated if applicable
    if (assignment.isTerminated) {
      const lastEventTime = events.length > 0 ? events[events.length - 1].createdAt : (assignment.completedAt || new Date());
      timeline.push({
        id: "sys-terminated",
        type: "auto_terminated",
        details: "System automatically terminated exam due to reaching malpractice limit",
        createdAt: new Date(lastEventTime.getTime() + 1000), // 1s after last violation
      });
    }
    ...
    return {
      ...
      assignment: {
        ...
        malpracticeCount: assignment.malpracticeCount,
        isTerminated: assignment.isTerminated,
    ...
    console.error("Failed to fetch submission audit logs:", error);
    return { success: false, error: "Failed to fetch logs" };
  }
}

export async function resumeExamSubmission(assignmentId: string, reason: string) {
  if (!reason || reason.trim().length === 0) {
    return { success: false, error: "Reason is required to resume attempt" };
  }

  try {
    const assignment = await db.query.examAssignments.findFirst({
      where: eq(examAssignments.id, assignmentId),
      columns: { examId: true, isTerminated: true, malpracticeCount: true },
    });
    ...
    const manageAccess = await ensureExamManageAccess(assignment.examId, "update");
    if (!manageAccess.examRecord) {
      return { success: false, error: "Unauthorized to manage this exam" };
    }

    if (!assignment.isTerminated) {
      return { success: false, error: "Cannot resume: attempt is not terminated." };
    }

    await db.insert(malpracticeEvents).values({
      assignmentId,
      actorId: manageAccess.access.session.user.id,
      type: "invigilator_resume",
    ...
    await db
      ...
      .set({
        isTerminated: false,
        status: "in_progress",
        malpracticeCount: Math.max(0, assignment.malpracticeCount - 1),
      })
      .where(eq(examAssignments.id, assignmentId));
    ...
    console.error("Failed to resume exam submission:", error);
    return { success: false, error: "Failed to resume attempt" };
  }
}

export async function resetExamSubmission(assignmentId: string, _reason?: string) {
  return deleteExamSubmission(assignmentId);
}

export async function getExamAllAuditLogs(examId: string) {
  try {
    const access = await ensureExamReadAccess(examId);
    if (!access.examRecord) {
    ...
    const logs = events.map((ev) => {
      const asgn = assignmentMap.get(ev.assignmentId);
      return {
        id: ev.id,
        studentName: asgn?.user?.name ?? "Unknown",
        studentEmail: asgn?.user?.email ?? "-",
        username: asgn?.user?.username ?? "-",
        eventType: ev.type,
        actorName: ev.actor?.name ?? "System / Student",
        createdAt: ev.createdAt,
        details: ev.details ?? "",
      };
    });
    ...
    console.error("Failed to get exam audit logs:", error);
